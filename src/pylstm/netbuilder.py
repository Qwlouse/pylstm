...
def ensure_unique_output_layer(layers):
    output_layers = [l for l in layers if l.layer_type == 'OutputLayer']
    assert len(output_layers) == 1, \
        "Found %d OutputLayers, but has to be 1." % len(output_layers)

    output_layer = output_layers[0]
    assert len(output_layer.targets) == 0, \
        "OutputLayer is not allowed to have targets."

    sink_layers = [l for l in layers if len(l.targets) == 0]
    assert len(sink_layers) == 1 and sink_layers[0] == output_layer,\
        "Only the OutputLayer is allowed to have empty targets list!"
...
def extend_architecture_info(architecture):
    ...
    extended_architecture['OutputLayer'] = {
        'size': 0,
        'type': 'OutputLayer',
        'targets': []
    }
