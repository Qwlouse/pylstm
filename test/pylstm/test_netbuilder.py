...
import unittest
from pylstm.netbuilder import NetworkBuilder, InvalidArchitectureError, ConstructionLayer
...
class ConstructionLayerTests(unittest.TestCase):
    ...
    def test_connecting_two_layers(self):
        cl1 = ConstructionLayer(Foo, 3)
        cl2 = ConstructionLayer(Foo, 4)
        cl1 >> cl2
    ...
    def test_connect_multiple_targets(self):
        l0, l1, l2, l3 = [ConstructionLayer(Foo, 10) for _ in range(4)]
        l0 >> l1
        l0 >> l2
        l0 >> l3
    ...
    def test_get_depth_linear_architecture(self):
        layers = [ConstructionLayer(Foo, 10) for _ in range(5)]
        for i in range(4):
            layers[i] >> layers[i+1]
        layers[0].depth = 0
    ...
    def test_traverse_circle_raises_error(self):
        l0, l1, l2 = [ConstructionLayer(Foo, 10) for _ in range(3)]
        l0 >> l1 >> l2 >> l0
        try:
...
class NetBuilderTests(unittest.TestCase):
    ...
    def test_topological_sort(self):
        cl = lambda : ConstructionLayer(Foo, 2)
        self.netb.input(3) >> cl() >> cl() >> cl() >> self.netb.output
        self.netb.input() >> cl() >> self.netb.output
