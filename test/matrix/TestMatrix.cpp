...
TEST_F(MatrixTest, check_if_add_scalar_works_as_expected)
{
  add_scalar(m1, 10.);
  ASSERT_TRUE(equals(m1, MatrixCPU({{12.,13.,15.},{13.,14.,65.}})));
}

TEST_F(MatrixTest, check_if_dot_works_as_expected)
{
  dot(m1, m3, m2);
...
TEST_F(MatrixTest, check_if_dot_add_works_as_expected)
{
  dot_add(m1, m3, m2);
  // check that m1 and m3 are unchanged
  ASSERT_TRUE(equals(m1, MatrixCPU({{2.,3.,5.},{3.,4.,55.}})));
...
TEST_F(MatrixTest, check_if_mult_works_as_expected)
{
  mult(m4, m1, m2);
  // check that m1 and m3 are unchanged
  ASSERT_TRUE(equals(m1, MatrixCPU({{2.,3.,5.},{3.,4.,55.}})));
...
TEST_F(MatrixTest, check_if_mult_add_works_as_expected)
{
  mult_add(m4, m1, m2);
  // check that m1 and m3 are unchanged
  ASSERT_TRUE(equals(m1, MatrixCPU({{2.,3.,5.},{3.,4.,55.}})));
